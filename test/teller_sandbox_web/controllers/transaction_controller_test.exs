  ...
  describe "get transactions" do
    test "transactions for account id not found", %{conn: conn} do
      response =
        ...
        |> get(Routes.transaction_path(conn, :get_transactions, "acc_12345"), %{
    ...
    test "transactions for account id found", %{conn: conn} do
      response =
        ...
        |> put_req_header("authorization", "Basic #{Base.url_encode64("user_12345:")}")
        |> get(
          Routes.transaction_path(conn, :get_transactions, "acc_78aae6a7fa0bfb67a00a"),
      ...
      assert Decimal.sub(
               Decimal.new(trans_1["running_balance"]),
               Decimal.negate(trans_1["amount"])
             ) == Decimal.new(trans_0["running_balance"])
    end
  end
end
